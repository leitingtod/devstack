...
function mysql_secure_installation_auto {
    local passwd=$1
    prepare
    find_mysql_client
    set_echo_compat
    ...
    if [ $hadpass -eq 0 ]; then
        echo $echo_n "Set root password? $echo_c"
    else
        echo "You already have a root password set."
        echo
        echo $echo_n "Change the root password? $echo_c"
    fi
